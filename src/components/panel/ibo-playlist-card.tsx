import { Download, ExternalLink, Files, WandSparkles } from "lucide-react";
...
export function IboPlaylistCard({ baseUrl, username }: IboPlaylistCardProps) {
  ...
  const handleCopy = async () => {
    try {
      ...
      toast.success("تم نسخ رابط IBO الثابت");
    } catch {
  ...
  const handleDownload = () => {
    ...
    toast.success("تم توليد ملف IBO محليًا للتنزيل");
  };

  return (
    <Card className="rounded-[2rem] border-white/10 bg-[#0b1228] shadow-[0_20px_60px_rgba(5,10,30,0.35)]">
      <CardHeader className="flex flex-row items-center justify-between pb-3">
        <CardTitle className="text-lg text-white">Generate IBO M3U</CardTitle>
        <WandSparkles className="h-5 w-5 text-[#f59e0b]" />
      </CardHeader>
      <CardContent className="space-y-4 text-right">
        <div className="rounded-3xl border border-emerald-500/20 bg-emerald-500/10 p-4 text-sm leading-7 text-emerald-100">
          النسخة الحالية تعتمد ملفًا ثابتًا عامًا للمستخدم <span dir="ltr">{username}</span> داخل <span dir="ltr">/public/ibo</span> لأنه ثبت أنه يعمل مع IBO Player على Vercel.
        </div>

        <div className="grid gap-3 rounded-3xl border border-white/10 bg-white/5 p-4 text-sm sm:grid-cols-2">
          <div>
            <p className="text-slate-400">Hosted IBO URL</p>
